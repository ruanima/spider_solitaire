...
class View:
    ...
    def __init__(self, parent, quit, **kwargs):
        ...
        self.circular = tk.Label(status, text=" Circular ", relief=tk.RIDGE,
                                 font=STATUS_FONT,  bg=STATUS_BG, fg='Black', bd=2)
        self.open = tk.Label(status, text=" Open     ", relief=tk.RIDGE,
                             font=STATUS_FONT, bg=STATUS_BG, fg='Black', bd=2)
        self.deals = tk.Label(status, relief=tk.RIDGE,
                              font=STATUS_FONT, bg=STATUS_BG, fg='Black', bd=2)
        self.down = tk.Label(status, relief=tk.RIDGE,
                             font=STATUS_FONT, bg=STATUS_BG, fg='Black', bd=2)
        self.moves = tk.Label(status, relief=tk.RIDGE,
                              font=STATUS_FONT, bg=STATUS_BG, fg='Black', bd=2)
        ...
        tableau = self.tableau = ScrolledCanvas(
            root, bg=BACKGROUND, cursor=DEFAULT_CURSOR, scrolls=tk.VERTICAL, **kwargs)
        ...
        tableau.create_text(width//2-MARGIN, 2.5*MARGIN, text='Undo',
                            fill=CELEBRATE, tag='undo', anchor=tk.CENTER)
        tableau.create_text(width//2+7*MARGIN, 2.5*MARGIN, text='Redo',
                            fill=CELEBRATE, tag='redo', anchor=tk.CENTER)
        self.loadImages()
        self.createCards()
        ...
        for w in self.waste:
            tableau.create_rectangle(
                w[0], w[1], w[0]+CARDWIDTH, w[1]+CARDHEIGHT, outline=OUTLINE)
        for f in self.foundations:
            tableau.create_rectangle(
                f[0], f[1], f[0]+CARDWIDTH, f[1]+CARDHEIGHT, outline=OUTLINE)
    ...
    def loadImages(self):
        ...
        for rank, suit in itertools.product(ALLRANKS, SUITNAMES):
            face = PhotoImage(file=os.path.join(
                cardDir, suit+RANKNAMES[rank]+'.gif'))
    ...
    def createCards(self):
        ...
        for card in model.deck:
            c = canvas.create_image(-200, -200, image=None,
                                    anchor=tk.NW, tag="card")
    ...
    def findOverlapping(self, seq, west, east):
        ...
        answer = [(pile, k)
                  for k, pile in enumerate(seq) if overlap(pile) >= 0]
    ...
    def showStats(self, stats):
        if stats == None:
            showerror('No Stats File', os.path.join(os.path.dirname(
                sys.argv[0]), 'stats.txt') + ' does not exist.')
        elif stats == []:
            showerror('No Stats to Display', os.path.join(
                os.path.dirname(sys.argv[0]), 'stats.txt') + ' is empty.')
        else:
...
class StatsDialog(SimpleDialog):
    def __init__(self, top, stats):
        ...
        super().__init__(top, text='', buttons=[
            "Dismiss"], default=0, cancel=0, title='Spider Stats')
